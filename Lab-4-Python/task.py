...
def get_memtable(stuffdict):
    ...
    table = np.array([[0 for _ in range(7+1)] for _ in range(n+1)])
    
    for row in range(n+1):
        for colmn in range(7+1):
...
def get_selected_item_list(stuffdict, back=7):
...
def get_backpack(stuffdict, row, column):
    nump = np.zeros((row,column), dtype='<U1')
    colmn = len(nump[0])
    n = colmn
    ...
    while len(keys) > 0:
        for i in keys:
            for j in range(colmn):
                ...
                if np.count_nonzero(nump[k] == i) == s:
                    keys_1.append(i)
                    break

                if s <= n and nump[k][j] == '' and i not in keys_1:
                    nump[k][j] = i
                    n -= 1
        ...
        if np.count_nonzero(nump[0] == '') == 0 and np.count_nonzero(nump[0] == '') == 0:
            break

    return nump

print(get_backpack(d,2,4))
